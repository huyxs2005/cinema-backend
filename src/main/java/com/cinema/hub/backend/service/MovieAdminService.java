...
public class MovieAdminService {
    ...
    public List<MovieAdminResponseDto> getAll(String keyword, String status, String genre, String durationOrder, String ageRating) {
        ...
        String normalizedKeywordNoAccent = normalizedKeyword != null ? removeAccents(normalizedKeyword) : null;
        ...
        String normalizedAgeRating = StringUtils.hasText(ageRating) ? ageRating.trim().toUpperCase(Locale.ROOT) : null;

        List<MovieAdminResponseDto> filtered = movies.stream()
                .filter(movie -> normalizedKeyword == null || matchesKeyword(movie, normalizedKeyword, normalizedKeywordNoAccent))
                ...
                .filter(movie -> normalizedAgeRating == null || normalizedAgeRating.equalsIgnoreCase(defaultString(movie.getAgeRating())))
    ...
    private boolean matchesKeyword(MovieAdminResponseDto movie, String normalized, String normalizedNoAccent) {
        return containsIgnoreCase(movie.getTitle(), normalized, normalizedNoAccent)
                || containsIgnoreCase(movie.getOriginalTitle(), normalized, normalizedNoAccent);
    }

    private boolean containsIgnoreCase(String value, String normalized, String normalizedNoAccent) {
        if (value == null) {
            return false;
        }
        ...
        if (normalizedNoAccent == null) {
            return false;
        }
        String collapsed = removeAccents(lower);
        return collapsed.contains(normalizedNoAccent);
    }
