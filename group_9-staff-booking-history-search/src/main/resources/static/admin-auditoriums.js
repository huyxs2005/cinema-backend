...
function bindAuditoriumFilters() {
    ...
    const resetBtn = document.getElementById("auditoriumFilterReset");
...
function validateAuditoriumPayload(payload) {
    ...
    if (!payload.numberOfRows || payload.numberOfRows <= 0) {
        errors.push({ field: "auditoriumRows", message: "Số hàng ghế phải lớn hơn 0 *" });
    } else if (payload.numberOfRows > AUD_MAX_VALUE) {
        errors.push({ field: "auditoriumRows", message: "Số hàng ghế tối đa 999 *" });
    }
    if (!payload.numberOfColumns || payload.numberOfColumns <= 0) {
        errors.push({ field: "auditoriumColumns", message: "Số ghế mỗi hàng phải lớn hơn 0 *" });
    } else if (payload.numberOfColumns > AUD_MAX_VALUE) {
        errors.push({ field: "auditoriumColumns", message: "Số ghế mỗi hàng tối đa 999 *" });
    }
    return errors;
}
...
async function editAuditorium(id) {
    try {
        ...
        document.getElementById("auditoriumFormTitle").textContent = `Chỉnh sửa phòng chiếu #${data.id}`;
        document.getElementById("auditoriumFormMessage").textContent = "";
        clearAuditoriumErrors();
        window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (error) {
