...
async function fetchMovies() {
    ...
    try {
        const response = await fetch(movieAdminApi.list);
...
function renderMovieTable(movies) {
    ...
    if (!Array.isArray(movies) || movies.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="text-center py-4 text-muted">Chưa có phim nào</td></tr>`;
        counter.textContent = "0 items";
        return;
    }
...
function fillMovieForm(movie) {
    document.getElementById("movieFormTitle").textContent = "Chỉnh sửa phim";
