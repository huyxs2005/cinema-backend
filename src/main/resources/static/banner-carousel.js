    ...
    fetch("/api/banners/active")
        ...
        .then((data) => {
            if (!Array.isArray(data) || data.length === 0) {
                renderPlaceholder("Không có banner nào dang ho?t d?ng");
                return;
            }
            ...
            sorted.forEach((banner, index) => {
                ...
                if (banner.linkType === "MOVIE") {
                    ...
                    altText = banner.promotionTitle
                        ? `Khuy?n mãi ${banner.promotionTitle}`
                        : "Banner khuy?n mãi HUB";
        ...
        .catch(() => renderPlaceholder("Không th? t?i banner. Vui lòng th? l?i sau."));
});
