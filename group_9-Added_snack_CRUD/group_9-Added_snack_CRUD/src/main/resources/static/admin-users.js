...
function renderUserTable(items) {
    ...
    items.forEach((item, index) => {
        ...
        const deleteButtonHtml = isSelf
            ? `<button type="button" class="btn btn-sm btn-outline-danger user-action-btn" disabled title="Không thể xóa tài khoản đang đăng nhập">Xóa</button>`
            : `<button type="button" class="btn btn-sm btn-outline-danger user-action-btn" data-user-delete="${item.userId}">Xóa</button>`;
        const toggleLabel = item.active ? "Vô hiệu hóa" : "Kích hoạt";
        const toggleClass = item.active ? "btn btn-sm btn-outline-warning user-action-btn" : "btn btn-sm btn-outline-success user-action-btn";
        tr.innerHTML = `
            <td>${index + 1 + userState.page * 10}</td>
            <td>${safeText(item.fullName)}</td>
            <td>${safeText(item.email)}</td>
            <td>${safeText(item.phone)}</td>
            <td>${safeText(item.role)}</td>
            <td>${renderUserStatus(item.active)}</td>
            <td>${formatDateTimeDisplay(item.lastLoginAt)}</td>
            <td>${formatDateTimeDisplay(item.createdAt)}</td>
            <td>
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-outline-light btn-sm" data-user-edit="${item.userId}">Sửa</button>
                    <button type="button" class="${toggleClass}" data-user-toggle="${item.userId}" data-active="${item.active}">${toggleLabel}</button>
                    ${deleteButtonHtml}
                </div>
            </td>
        `;
    ...
    tbody.querySelectorAll("[data-user-edit]").forEach((btn) =>
        btn.addEventListener("click", () => editUser(Number(btn.dataset.userEdit)))
    );
    tbody.querySelectorAll("[data-user-toggle]").forEach((btn) =>
        btn.addEventListener("click", () => toggleUserActive(Number(btn.dataset.userToggle), btn.dataset.active !== "true"))
    );
    tbody.querySelectorAll("[data-user-delete]").forEach((btn) =>
        btn.addEventListener("click", () => confirmDeleteUser(Number(btn.dataset.userDelete)))
    );
}
